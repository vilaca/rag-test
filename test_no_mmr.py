#!/usr/bin/env python3
"""Test script to verify DRY question handling without MMR diversity."""
...
def test_dry_no_mmr():
    """Test the DRY question without MMR diversity."""
    print("Testing DRY question handling without MMR diversity...")
    ...
    try:
        ...
        # Temporarily disable MMR diversity
        original_should_use_diversity = rag._should_use_diversity
        rag._should_use_diversity = lambda query: False
        
        context = rag.retrieve(question)
        print(f"Retrieved {len(context)} chunks (no MMR):")
        ...
        rag._should_use_diversity = original_should_use_diversity
        
        # Test with MMR enabled
        print(f"\n=== With MMR Diversity ===")
        context_with_mmr = rag.retrieve(question)
        print(f"Retrieved {len(context_with_mmr)} chunks (with MMR):")
        
        for i, chunk in enumerate(context_with_mmr):
            has_dry = "DRY" in chunk or "don't repeat yourself" in chunk.lower()
            print(f"  {i+1}. Has DRY: {has_dry}")
            if has_dry:
                print(f"     🎯 {chunk[:150]}...")
